<?php
// Header común para panel admin y cuenta
?>
<header class="h-[60px] px-6 border-b border-slate-200 bg-white/95 backdrop-blur-sm flex items-center justify-between shadow-sm sticky top-0 z-10">
  ...
  <div class="flex items-center gap-3 min-w-0">
    <div id="page-title" class="flex items-center gap-2">
      <?php if (isset($pageTitle)): ?>
        <span class="text-sm font-medium text-slate-700 truncate"><?php echo htmlspecialchars($pageTitle); ?></span>
      <?php endif; ?>
    </div>
  </div>
    ...
    <div class="relative" id="profile-dropdown-container">
      ...
      <div id="profile-dropdown" class="hidden absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50">
        <div class="px-4 py-3 border-b border-slate-100">
          <div id="session-user" class="font-semibold text-slate-800 text-sm">
            <?php echo htmlspecialchars(($user['first_name'] ?? '') . ' ' . ($user['last_name'] ?? '')); ?>
          </div>
          <div id="session-meta" class="text-xs text-slate-500 mt-0.5">
